...
Highest number: \[Your answer here\] Lowest number: \[Your answer here\]
...
rating_summary_d = rating_hyp_long_d %>%
  ## your code here
  group_by(ConfrontationalGame, Emotion, IsRecall) %>%
...
performance_hyp_long_d %>%
  mutate(
    WithMusic = factor(
      WithMusic, 
      levels=c(F, T), 
      labels=c("PreMusic", "PostMusic")
    )
  ) %>%
  select(-c("Score", "Measurement")) %>%
  pivot_wider(
    names_from = WithMusic, 
